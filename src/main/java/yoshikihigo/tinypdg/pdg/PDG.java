...
public class PDG implements Comparable<PDG> {
	...
	public PDG(final MethodInfo unit, final PDGNodeFactory pdgNodeFactory,
			...
			final boolean buildExecutionDependency,String authorName) {
...
//		}
	...
	public void build() {
		...
		if (this.buildControlDependence) {  //add edges for enter parameter node
		...
		if (this.buildExecutionDependence) {//cfg enternode和method enternode的运行依赖
			if (!this.cfg.isEmpty()) {
		...
		if (this.buildDataDependence) {//参数的数据依赖
			for (final PDGParameterNode parameterNode : this.parameterNodes) {
				if (!this.cfg.isEmpty()) {
		...
		if (!this.cfg.isEmpty()) {  // add edges for enternode
			this.buildDependence(this.cfg.getEnterNode(), checkedNodes);
		}

		for (final CFGNode<?> cfgExitNode : this.cfg.getExitNodes()) {  //add edges for exit node
	...
	public int getAllNodesBeforeMergeCount() {
		int count = 0;
		for(PDGNode node : this.getAllNodes()){
			if(node instanceof PDGMergedNode){
				count += ((PDGMergedNode) node).getOriginalNodes().size();
			}
			else{
				count++;
			}
