...
public class CompareTwo {
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            {
                final Option f = new Option("f", "function", true,
                        "changed function");
                f.setArgName("function");
                f.setArgs(1);
                f.setRequired(true);
                options.addOption(f);
            }
            ...
            final File changedFunction = new File(cmd.getOptionValue("f"));
            if (!target.exists()) {
                System.err
                        .println("specified directory or file does not exist.");
                System.exit(0);
            }
            if (!changedFunction.exists()) {
                System.err
                        .println("specified changed function file does not exist.");
                System.exit(0);
            }
...
//            Process process = Runtime.getRuntime().exec("git clone https://github.com/cs2103aug2014-f10-3j/Epiphany.git");
//            process.waitFor();
//            Process process = Runtime.getRuntime().exec("cd Epiphany && git log --name only > commit.txt");
            Stack<CommitRecord> commitStack = new Stack<>();
            String filePath = "./Epiphany/commit.txt";
            getCommitRecord(filePath,commitStack);
            PDG[] changedPdgArray;
            PDG[] targetPdgArray;
...
//
            while(!commitStack.empty()){
                final CommitRecord record = commitStack.peek();
                //generatePDGs(record.author,record.filePath,changedPdgArray);
                final List<File> changedfiles = new ArrayList<>();
                for(String file:record.filePath){
                    changedfiles.add(new File(file));
                }
                ...
                for (int i = 0; i < pdgGenerationThreads.length; i++) {
                    pdgGenerationThreads[i] = new Thread(
                            new PDGGenerationThread(methods, pdgs,
                                    ...
                                    useOfMerging, SIZE_THRESHOLD,null));
                ...
                changedPdgArray = pdgs.toArray(new PDG[0]);
            ...
            System.out.print("generating PDGs for target directory and changed functions ... ");

            //final PDG[] changedPdgArray;

            {
                final List<File> changedfiles = getFiles(changedFunction);
                final List<MethodInfo> methods = new ArrayList<MethodInfo>();
                for (final File file : changedfiles) {
                    final CompilationUnit unit = TinyPDGASTVisitor
                            .createAST(file);
            ...
            //final PDG[] targetPdgArray;
            {
                final List<File> targetfiles = getFiles(target);
                final List<MethodInfo> methods = new ArrayList<MethodInfo>();
                for (final File file : targetfiles) {
                    ...
                    final TinyPDGASTVisitor visitor = new TinyPDGASTVisitor(
                            file.getAbsolutePath(), unit, methods);
                    unit.accept(visitor);//traverse AST
            ...
            System.out.print("calculating hash values of target directory and changed function ... ");//normalized and get hash values

            final PDG[] combinePdgArray = new PDG[targetPdgArray.length + changedPdgArray.length];
                    ...
                    .synchronizedSortedSet(new TreeSet<ClonePairInfo>());
            {
                ...
                for(int i = 0;i < changedPdgArray.length;i++) {
                    for (int j = 0; j < targetPdgArray.length; j++) {
                        pdgpairs.add(new PDGPairInfo(changedPdgArray[0], targetPdgArray[j]));
                    }
                }
    ...
    private void generatePDGs(String author,String[] filePath,PDG[] changedPdgArray){

    }

    private static void getCommitRecord(String filePath,Stack<CommitRecord> commitStack) throws IOException {
        ...
        while((str = bufferedReader.readLine()) != null)
        {
            ...
            else if(!str.startsWith(" ") && !str.startsWith("Date:")){
