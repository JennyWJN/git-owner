...
public class Slicing {
	...
	public Slicing(final String pathA, final String pathB,
			...
			final SortedSet<NodePairInfo> checkedNodepairs, final String authorA,final String authorB,
				   final String methodA, final String methodB, final int compareNumber) {
	...
	public ClonePairInfo perform() {
		if (null == this.clonepair) {
			...
			this.clonepair = this.perform(this.startNodeA, this.startNodeB,
					checkedNodesA, checkedNodesB,this.authorA,this.authorB, 0,0);
		}
	...
	private ClonePairInfo perform(final PDGNode<?> nodeA,
			...
			final SortedSet<PDGNode<?>> checkedNodesB, String authorA, String authorB,final int sizeA,final int sizeB) {

		final NodePairInfo nodepair = new NodePairInfo(nodeA, nodeB);
		if (this.checkedNodepairs.contains(nodepair)) {//if has been checked,return empty clonepair
			return new ClonePairInfo(this.pathA, this.pathB, authorA, authorB,sizeA,sizeB,this.compareNumber,methodA,methodB);
		}
		...
		bSortedEdgesA.addAll(bEdgesA);//generate sorted backward edges
		...
		ClonePairInfo clonepair = new ClonePairInfo(this.pathA, this.pathB, this.authorA, this.authorB,0 ,0,this.compareNumber,methodA,methodB);
		for (final ClonePairInfo candidate : candidates) {
			if (clonepair.size() < candidate.size()) {
				clonepair = candidate;//choose candidate with most nodepairs
			}
		}
	...
	private void makeCandidates(final List<ClonePairInfo> candidates,
		...
		for (final ClonePairInfo clonepair : clonepairs) {
			for (final ClonePairInfo candidate : candidates) {
				if (!candidate.conflict(clonepair)) {//if no same element，merge
					candidate.merge(clonepair);
				}
			}
			final ClonePairInfo newCandidate = new ClonePairInfo(this.pathA,
					this.pathB, this.authorA, this.authorB, 0, 0,this.compareNumber,
					methodA,methodB);
			...
			EDGEB: for (final PDGEdge edgeB : edgesB) {
				...
				if (equivalentNodesA == equivalentNodesB) {
					...
					final ClonePairInfo clonepair = this.perform(
							...
							newCheckedNodesB,this.authorA,this.authorB, 0, 0);
			...
			EDGEB: for (final PDGEdge edgeB : edgesB) {
				...
				if (equivalentNodesA == equivalentNodesB) {
					...
					final ClonePairInfo clonepair = this.perform(edgeA.toNode,
							edgeB.toNode, newCheckedNodesA, newCheckedNodesB,this.authorA,this.authorB, 0, 0);
					clonepairs.add(clonepair);
				}
			}
		}
